    ...
    def auto_unfollow(self):
        ...
        if self.login_status:
            ...
            if self.login_status == 1:
                ...
                try:
                    r = self.s.get(url_tag).text #agregue .text
                    #print(r.text)
                    #all_data = json.loads(r.text)

                    #user_info = all_data['graphql']['user']
                    user_info = r[r.find('javascript">window._sharedData'): r.find('<script type', r.find(
                        'javascript">window._sharedData'))] #agregado
                    log_string = "Checking user info.."
                    self.write_log(log_string)

                    #follows = user_info['edge_follow']['count']
                    follows = get_str_info(user_info, '"edge_follow":{"count":', '}','n') #agregado
                    #follower = user_info['edge_followed_by']['count']
                    follower = get_str_info(user_info, 'edge_followed_by":{"count":', '}','n') #agregado
                    #media = user_info['edge_owner_to_timeline_media']['count']
                    media = get_str_info(user_info, 'edge_owner_to_timeline_media":{"count":', ',','n') #agregado
                    #follow_viewer = user_info['follows_viewer']
                    follow_viewer = get_str_info(user_info, 'follows_viewer":', ',','b') #agregado
                    #followed_by_viewer = user_info[
                    #    'followed_by_viewer']
                    followed_by_viewer = get_str_info(user_info, 'followed_by_viewer":', ',','b') #agregado
                    #requested_by_viewer = user_info[
                    #    'requested_by_viewer']
                    requested_by_viewer = get_str_info(user_info, 'requested_by_viewer":', ',','b') #agregado
                    #has_requested_viewer = user_info[
                    #    'has_requested_viewer']
                    has_requested_viewer = get_str_info(user_info, 'has_requested_viewer":', ',','b') #agregado
